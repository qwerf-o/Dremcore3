...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateClouds() {
    ...
    cloudData.forEach((data) => {
      ...
      const autonomousX =
        Math.sin(time * data.frequencyX + data.phaseX) * data.autonomousAmpX;
      const autonomousY =
        Math.cos(time * data.frequencyY + data.phaseY) * data.autonomousAmpY;

      // Calculate rotation based on mouse position with slight randomness
      const baseRotationAdjustment = (lastMouseX - 50) * 0.03;
      const targetRotation =
        data.baseRotation +
        baseRotationAdjustment +
      ...
      data.currentRotation +=
        (targetRotation - data.currentRotation) / (data.delay * 1.5);
...
const Infos = {
  ...
  createInfo: function (message) {
    ...
    if (message.includes("...")) {
      ...
      infoMessage.innerHTML = message.replace(
        "...",
    ...
    info.querySelector(".info-close-text").onclick = function () {
      ...
      const infoMessage = info.querySelector(".info-message");
      const userDream = infoMessage.textContent
        .replace("Мне снилось, что ", "")
        .trim();

      // Check if the placeholder was filled (not empty and not "...")
      if (userDream && userDream !== "...") {
        alert(
          `Спасибо за ваш ответ! Скоро ваш сон "${userDream}" станет реальностью.`
        );
      }
...
const CharEditor = {
  ...
  init: function () {
    ...
    canva.addEventListener(
      "mousedown",
      this.handleCanvaElementClick.bind(this)
    );
  ...
  handleDrop: function (e) {
    ...
    if (
      e.target.className === "canva" ||
      e.target.parentElement.className === "canva"
    ) {
      const canva =
        e.target.className === "canva" ? e.target : e.target.parentElement;
      ...
      if (!this.isDraggingFromCanva) {
        ...
        clone.style.left =
          e.clientX - canvaRect.left - clone.offsetWidth / 2 + "px";
        clone.style.top =
          e.clientY - canvaRect.top - clone.offsetHeight / 2 + "px";
        ...
        this.draggedElement.style.left =
          e.clientX -
          canvaRect.left -
          this.draggedElement.offsetWidth / 2 +
          "px";
        this.draggedElement.style.top =
          e.clientY -
          canvaRect.top -
          this.draggedElement.offsetHeight / 2 +
          "px";
      ...
      this.canvaElements = this.canvaElements.filter(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function makeDialogDraggable(dialog, handle) {
    ...
    function startDrag(e) {
      ...
      if (
        e.target.closest(".warning-close") ||
        e.target.closest(".info-close-text")
      ) {
        return;
      }
  ...
  if (charEditor) {
    ...
    smiley.addEventListener("mouseenter", function () {
      ...
      messageContainer.style.left =
        smiley.getBoundingClientRect().left + 50 + "px";
    });
